...
```
Постороим график этой функции. Так как функция имеет точку разрыва x = 0, во избежание вертикальной линии в точке разрыва разобьём график функции на два отрезка, и посторим их поочереди.
```python
...
```
Посторим касательную к графику в произвольно взятой точке x0 = 1. Для этого найдём значение y0 и значение производной в точке x0. Подставим значение в уравнение касательной: y = y0 + f`(x0) * (x - x0), получив множество элементов y для касательной.
```python
x0 = 1
...
plt.plot(x0, y0, 'ro')
```
Зададим название графика, подпишем оси координат, включим отображение легенды и сетки. Также создадим аннотацию к точке касание касательной, в ней укажем координаты этой точки, для удобства значение y округлим до двух знаков после запятой, зададим координаты блока текста аннотации и стиль стрелки.
```python
